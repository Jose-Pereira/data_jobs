    ...
    for k in range(0, config['rounds']):
        for query in config['search_queries']:
            ...
            for i in range (0, config['pages_to_scrape']):
                ...
                # Break if we've reached the job limit
                if len(all_jobs) >= job_limit:
                    print(f"Reached job limit of {job_limit}. Stopping scraping.")
                    break
            # Break outer loop if job limit reached
            if len(all_jobs) >= job_limit:
                break
        # Break outermost loop if job limit reached
        if len(all_jobs) >= job_limit:
            break
